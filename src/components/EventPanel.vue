    ...
    <div v-if="activeEvent" class="fixed inset-0 z-40 flex items-center justify-center bg-black/70 px-4">
      <section
        ...
        <div class="flex flex-col gap-3">
          <button v-for="choice in visibleChoices" :key="choice.id"
            class="rounded border border-gray-400 bg-neutral-700 px-4 py-3 text-left text-white transition hover:bg-neutral-500 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-neutral-700"
            :disabled="!eventStore.isChoiceAffordable(choice)" @click="onChoose(choice.id)">
